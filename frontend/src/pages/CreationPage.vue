...
<template>
  <section class="page workspace creation-layout" :class="{ 'history-open': historyOpen }">
    <ConversationHistoryDrawer v-model:open="historyOpen" title="Listing 创作" :items="historyItems" :selected-id="snapshot?.state.thread_id" :busy="sending" @create="createSession" @select="loadSession" @rename="renameSession" @delete="deleteSession"/>
